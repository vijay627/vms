...
Jun 09, 2010.
*/
"use strict";var REDIPS={};REDIPS.drag=(function(){var init,img_onmousemove,handler_onmousedown,handler_onmouseup,handler_onmousemove,handler_onresize,set_trc,set_color,box_offset,calculate_cells,getScrollPosition,autoscrollX,autoscrollY,clone_object,relocate_object,clone_limit,elementControl,trash_delete,enable_drag,get_style,in_scrollable,save_content,obj_margin=null,mouseX=null,mouseY=null,window_width=0,window_height=0,scroll_width=null,scroll_height=null,edge={page:{x:0,y:0},div:{x:0,y:0},flag:{x:0,y:0}},scroll_object,bgcolor_old=null,scrollable_container=[],tables=[],moved_flag=0,cloned_flag=0,cloned_id=[],currentCell=[],div_drag=null,div_box=null,table=null,table_old=null,table_source=null,row=null,row_old=null,row_source=null,cell=null,cell_old=null,cell_source=null,obj=false,obj_old=false,hover_color='#E7AB83',bound=25,speed=20,only={div:[],cname:'only',other:'deny'},mark={action:'deny',cname:'mark',exception:[]},border_disabled='dotted',trash='trash',trash_ask=true,drop_option='multiple',delete_cloned=true,target_cell=null,source_cell=null,clone_ctrlKey=false;init=function(){var self=this,i,j,imgs,evt,element,table_nested,tables_nodeList,divs_nodeList,obj_new_div;div_drag=document.getElementById('drag');if(!document.getElementById('obj_new')){obj_new_div=document.createElement('div');obj_new_div.id='obj_new';obj_new_div.style.width=obj_new_div.style.height='1px';div_drag.appendChild(obj_new_div);}
tables_nodeList=div_drag.getElementsByTagName('table');for(i=0,j=0;i<tables_nodeList.length;i++){table_nested=true;element=tables_nodeList[i].parentNode;do{if(element.id==='drag'){table_nested=false;break;}
if(element.nodeName==='TABLE'){break;}
element=element.parentNode;}while(element);if(!table_nested){tables[j]=tables_nodeList[i];j++;}}
handler_onresize();window.onresize=handler_onresize;enable_drag('init');imgs=div_drag.getElementsByTagName('img');for(i=0;i<imgs.length;i++){imgs[i].onmousemove=img_onmousemove;}
div_drag.onselectstart=function(e){evt=e||window.event;if(!elementControl(evt)){if(evt.ctrlKey){document.selection.clear();}
return false;}};window.onscroll=calculate_cells;};img_onmousemove=function(){return false;};handler_onmousedown=function(e){var evt=e||window.event,offset,mouseButton;if(elementControl(evt)){return true;}
...
offset=box_offset(obj);obj_margin=[mouseY-offset[0],offset[1]-mouseX,offset[2]-mouseY,mouseX-offset[3]];return false;};handler_onmouseup=function(e){var evt=e||window.event,target_table,i,target_elements,target_elements_length;if(obj.releaseCapture){obj.releaseCapture();}
document.onmousemove=null;document.onmouseup=null;obj.style.left=0;obj.style.top=0;obj.style.zIndex=10;scroll_width=document.documentElement.scrollWidth;scroll_height=document.documentElement.scrollHeight;edge.flag.x=edge.flag.y=0;if(cloned_flag===1&&(table===null||row===null||cell===null)){obj.parentNode.removeChild(obj);cloned_id[obj_old.id]-=1;REDIPS.drag.myhandler_notcloned();}
...
else if(moved_flag===0){REDIPS.drag.myhandler_moved();set_color();if(in_scrollable(obj)){relocate_object();}}
...
break;}}};set_color=function(){if(table!==null&&row!==null&&cell!==null){bgcolor_old=tables[table].rows[row].cells[cell].style.backgroundColor;tables[table].rows[row].cells[cell].style.backgroundColor=REDIPS.drag.hover_color;table_old=table;row_old=row;cell_old=cell;}};box_offset=function(box,type){var scrollPosition=getScrollPosition(),oLeft=0-scrollPosition[0],oTop=0-scrollPosition[1],box_old=box;if(type===undefined){do{oLeft+=box.offsetLeft-box.scrollLeft;oTop+=box.offsetTop-box.scrollTop;box=box.offsetParent;}
while(box&&box.nodeName!=='BODY');}
else{do{oLeft+=box.offsetLeft;oTop+=box.offsetTop;box=box.offsetParent;}
while(box&&box.nodeName!=='BODY');}
return[oTop,oLeft+box_old.offsetWidth,oTop+box_old.offsetHeight,oLeft];};calculate_cells=function(){var i,j,row_offset,cb;for(i=0;i<tables.length;i++){row_offset=[];for(j=tables[i].rows.length-1;j>=0;j--){row_offset[j]=box_offset(tables[i].rows[j]);}
tables[i].offset=box_offset(tables[i]);tables[i].row_offset=row_offset;}
div_box=box_offset(div_drag);for(i=0;i<scrollable_container.length;i++){cb=box_offset(scrollable_container[i].div,1);scrollable_container[i].offset=cb;scrollable_container[i].midstX=(cb[1]+cb[3])/2;scrollable_container[i].midstY=(cb[0]+cb[2])/2;}};getScrollPosition=function(){var scrollX,scrollY;if(typeof(window.pageYOffset)==='number'){scrollX=window.pageXOffset;scrollY=window.pageYOffset;}
...
obj.style.left=(pos+scrollPosition-old)+"px";mouseX-=scrollPosition-old;}
...
obj.style.top=(pos+scrollPosition-old)+"px";mouseY-=scrollPosition-old;}
...
obj_new.style.zIndex=999;offset=box_offset(obj);offset_dragged=box_offset(obj_new);obj_new.style.top=(offset[0]-offset_dragged[0])+"px";obj_new.style.left=(offset[3]-offset_dragged[3])+"px";obj_new.onmousedown=handler_onmousedown;obj_new.className=obj_new.className.replace('clone','');if(cloned_id[obj.id]===undefined){cloned_id[obj.id]=0;}
obj_new.id=obj.id+'c'+cloned_id[obj.id];cloned_id[obj.id]+=1;mouseX-=parseInt(obj_new.style.left,10);mouseY-=parseInt(obj_new.style.top,10);REDIPS.drag.obj_old=obj_old=obj;REDIPS.drag.obj=obj=obj_new;};relocate_object=function(){var offset,offset_after;offset=box_offset(obj);document.getElementById('obj_new').appendChild(obj);if(obj.setCapture){obj.setCapture();}
offset_after=box_offset(obj);obj.style.top=(offset[0]-offset_after[0])+"px";obj.style.left=(offset[3]-offset_after[3])+"px";mouseX-=parseInt(obj.style.left,10);mouseY-=parseInt(obj.style.top,10);};clone_limit=function(){var match_arr,limit_type,limit,classes;classes=obj_old.className;match_arr=classes.match(/climit(\d)_(\d+)/);if(match_arr!==null){limit_type=parseInt(match_arr[1],10);limit=parseInt(match_arr[2],10);limit-=1;classes=classes.replace(/climit\d_\d+/g,'');if(limit<=0){classes=classes.replace('clone','');if(limit_type===2){classes=classes.replace('drag','');obj_old.onmousedown=null;obj_old.style.cursor='auto';REDIPS.drag.myhandler_clonedend2();}
...
REDIPS.drag.myhandler_undeleted();}};enable_drag=function(enable_flag,div_id){var i,j,divs=[],borderStyle,cursor,handler,overflow,cb;if(enable_flag===true||enable_flag==='init'){handler=handler_onmousedown;borderStyle='solid';cursor='move';}
else{handler=null;borderStyle=REDIPS.drag.border_disabled;cursor='auto';}
if(div_id===undefined){divs=div_drag.getElementsByTagName('div');}
else{divs[0]=document.getElementById(div_id);}
for(i=0,j=0;i<divs.length;i=i+1){if(divs[i].className.indexOf('drag')>-1){divs[i].onmousedown=handler;divs[i].style.borderStyle=borderStyle;divs[i].style.cursor=cursor;divs[i].enabled=enable_flag;}
else if(enable_flag==='init'){overflow=get_style(divs[i],'overflow');if(overflow!=='visible'){divs[i].onscroll=calculate_cells;cb=box_offset(divs[i],1);scrollable_container[j]={div:divs[i],offset:cb,midstX:(cb[1]+cb[3])/2,midstY:(cb[0]+cb[2])/2};j++;}}}};get_style=function(el,style_name){var val;if(el.currentStyle){val=el.currentStyle[style_name];}
else if(window.getComputedStyle){val=document.defaultView.getComputedStyle(el,null).getPropertyValue(style_name);}
return val;};in_scrollable=function(el){do{el=el.parentNode;if(get_style(el,'overflow')!=='visible'){return true;}}
while(el.id!=='drag'&&el.nodeName!=='BODY');return false;};save_content=function(tbl){var query='',tbl_rows,cells,tbl_cell,t,r,c,d;if(tbl===undefined||tbl<0||tbl>tables.length-1){tbl=0;}
